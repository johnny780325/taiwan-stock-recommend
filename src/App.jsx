...
// IPO 新股申購頁
// ══════════════════════════════════════════════════════════════
function IPOView({ ipoData, loading }) {
  ...
  const rowStyle = (row) => {
    ...
    try {
      ...
      if (p.length === 3) {
        ...
        if (d > new Date()) return { background:"rgba(0,119,255,0.07)", borderColor:"rgba(0,119,255,0.15)" };
      }
    } catch {}
    if (roi >= 100) return { background:"rgba(255,209,102,0.06)", borderColor:"rgba(255,209,102,0.2)" };
    if (roi >= 30)  return { background:"rgba(6,214,160,0.05)",   borderColor:"rgba(6,214,160,0.15)" };
    if (roi < 0)    return { background:"rgba(224,82,82,0.05)",   borderColor:"rgba(224,82,82,0.15)" };
    return {};
  };
  ...
  return (
    <div style={{minHeight:"100vh",background:"#0a0a0f",color:"#fff",fontFamily:"'Noto Sans TC','PingFang TC',sans-serif"}}>
      <div style={{padding:"16px 16px 10px",borderBottom:"1px solid rgba(255,255,255,0.05)",position:"sticky",top:0,background:"#0a0a0f",zIndex:10}}>
        <div style={{fontSize:10,color:"#00d296",fontWeight:700,letterSpacing:2,marginBottom:2}}>TAIWAN STOCK</div>
        <div style={{fontSize:18,fontWeight:900,marginBottom:10}}>🎲 新股申購專區</div>
        <div style={{position:"relative"}}>
          <span style={{position:"absolute",left:12,top:"50%",transform:"translateY(-50%)",fontSize:14,color:"#444",pointerEvents:"none"}}>🔍</span>
          <input className="search-box" placeholder="搜尋代號、公司、市場..."
            value={search} onChange={e => setSearch(e.target.value)}
            style={{paddingLeft:36}}/>
        </div>
        <div style={{fontSize:10,color:"#444",marginTop:6}}>
          🔵 尚未抽籤 &nbsp; 🟡 報酬率≥100% &nbsp; 🟢 報酬率≥30% &nbsp; 🔴 虧損
        </div>
      </div>
      <div style={{padding:"8px 16px 72px",overflowX:"auto"}}>
        {loading ? (
          <div style={{textAlign:"center",paddingTop:60,color:"#555"}}>載入中...</div>
        ) : filtered.length === 0 ? (
          <div style={{textAlign:"center",paddingTop:60,color:"#555"}}>暫無資料</div>
        ) : (
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:12,minWidth:900}}>
            <thead>
              <tr>
                {cols.map(c => (
                  <th key={c} style={{background:"rgba(255,255,255,0.04)",color:"#555",fontWeight:700,padding:"8px 10px",textAlign:"left",borderBottom:"1px solid rgba(255,255,255,0.07)",whiteSpace:"nowrap",fontSize:10,letterSpacing:0.5}}>
                    {c}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {filtered.map((row, i) => {
                ...
                const roi = parseFloat(String(row["報酬率%"]||"0").replace(/[^0-9.-]/g,"")) || 0;
                return (
                  <tr key={i} style={{background:rs.background,borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
                    {cols.map(c => {
                      let val = row[c] || "─";
                      let style = {padding:"9px 10px",whiteSpace:"nowrap",color:"#ccc",fontSize:12};
                      if (c === "報酬率%") {
                        style.color = roi >= 0 ? "#e05252" : "#06d6a0";
...
function GiftView({ giftData, loading }) {
  ...
  return (
    <div style={{minHeight:"100vh",background:"#0a0a0f",color:"#fff",fontFamily:"'Noto Sans TC','PingFang TC',sans-serif"}}>
      <div style={{padding:"16px 16px 10px",borderBottom:"1px solid rgba(255,255,255,0.05)",position:"sticky",top:0,background:"#0a0a0f",zIndex:10}}>
        <div style={{fontSize:10,color:"#00d296",fontWeight:700,letterSpacing:2,marginBottom:2}}>TAIWAN STOCK</div>
        <div style={{fontSize:18,fontWeight:900,marginBottom:10}}>🎁 股東會紀念品專區</div>
        <div style={{position:"relative"}}>
          <span style={{position:"absolute",left:12,top:"50%",transform:"translateY(-50%)",fontSize:14,color:"#444",pointerEvents:"none"}}>🔍</span>
          <input className="search-box" placeholder="搜尋代號、公司、紀念品、地點..."
            value={search} onChange={e => setSearch(e.target.value)}
            style={{paddingLeft:36}}/>
        </div>
        <div style={{fontSize:10,color:"#444",marginTop:6}}>
      ...
      <div style={{padding:"8px 16px 72px",overflowX:"auto"}}>
        {loading ? (
          ...
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:12,minWidth:900}}>
            ...
            <tbody>
              {filtered.map((row, i) => {
                const days = daysLeft(row["最後買進日"]);
                const hasGift = row["紀念品"] && row["紀念品"] !== "─" && !row["紀念品"].includes("無");
                const urgentBg = days !== null && days <= 7  ? "rgba(224,82,82,0.07)"
                               : days !== null && days <= 14 ? "rgba(255,209,102,0.05)"
                               : hasGift ? "rgba(6,214,160,0.03)" : "transparent";
                return (
                  <tr key={i} style={{background:urgentBg,borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
                    {cols.map(c => {
                      let val = row[c] || "─";
                      let style = {padding:"9px 10px",whiteSpace:"nowrap",color:"#ccc",fontSize:12};
                      if (c === "紀念品") {
                        style.color = hasGift ? "#00d296" : "#555";
                        style.fontWeight = hasGift ? 700 : 400;
                        style.maxWidth = 200;
                      ...
                      if (c === "股票代號") { style.color = "#4da6ff"; style.fontFamily = "monospace"; }
                      if (c === "公司名稱") { style.color = "#fff"; style.fontWeight = 600; }
                      if (c === "股價") { style.color = "#ffd166"; style.fontFamily = "monospace"; }
                      return <td key={c} style={style}>{val}</td>;
                    })}
                  </tr>
                );
...
export default function App() {
  // ── State ────────────────────────────────────────────────────
  const [page,       setPage]       = useState("main");   // "main" | "ipo" | "gift"
  const [sideOpen,   setSideOpen]   = useState(false);    // 預設收合
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (json.ipo)  setIpoData(json.ipo);
      if (json.gift) setGiftData(json.gift);
      setDebugMsg("");
    ...
    <div style={{display:"flex",height:"100vh",overflow:"hidden",background:"#0a0a0f",color:"#fff",fontFamily:"'Noto Sans TC','PingFang TC',sans-serif"}}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;} body{background:#0a0a0f;} html,body{-webkit-text-size-adjust:none;} input{font-size:16px!important;}
        ...
        /* 側邊欄 */
        .sidebar{background:#0d0f14;border-right:1px solid rgba(255,255,255,0.06);display:flex;flex-direction:column;flex-shrink:0;overflow:hidden;transition:width 0.22s cubic-bezier(.4,0,.2,1);}
        .nav-item{display:flex;align-items:center;gap:10px;padding:12px 14px;cursor:pointer;border-left:3px solid transparent;transition:all 0.15s;white-space:nowrap;overflow:hidden;user-select:none;}
        .nav-item:hover{background:rgba(255,255,255,0.04);}
        .nav-item.active{background:rgba(0,210,150,0.07);border-left-color:#00d296;}
        .nav-item.active .nav-label{color:#00d296;font-weight:700;}
        .nav-icon{font-size:18px;flex-shrink:0;width:22px;text-align:center;}
        .nav-label{font-size:13px;color:#888;}
        .toggle-btn{background:none;border:none;color:#555;cursor:pointer;padding:4px;border-radius:6px;display:flex;align-items:center;justify-content:center;font-size:16px;transition:color 0.15s;flex-shrink:0;}
        .toggle-btn:hover{color:#aaa;}
      `}</style>

      {/* ── 側邊欄 ── */}
      <div className="sidebar" style={{width: sideOpen ? 190 : 48}}>
        {/* 展開/收合按鈕 */}
        <div style={{padding:"14px 12px",borderBottom:"1px solid rgba(255,255,255,0.05)",display:"flex",alignItems:"center",gap:8,minHeight:52}}>
          <button className="toggle-btn" onClick={() => setSideOpen(o => !o)} title="選單">
            {sideOpen ? "◀" : "☰"}
          </button>
          {sideOpen && <span style={{fontSize:12,fontWeight:700,color:"#00d296",whiteSpace:"nowrap",overflow:"hidden"}}>台股選股雷達</span>}
        </div>
        {/* 導覽項目 */}
        ...
        <div style={{padding:"10px 8px",borderTop:"1px solid rgba(255,255,255,0.05)"}}>
          <button className="upd-btn" onClick={loadData} disabled={isLoading}
            style={{width: sideOpen ? "100%" : 32, height:32, padding:0, borderRadius:8, fontSize: sideOpen ? 11 : 14, display:"flex",alignItems:"center",justifyContent:"center",gap:4, transition:"width 0.22s"}}>
            <span>{isLoading ? "⏳" : "🔄"}</span>
            {sideOpen && <span>{isLoading?"載入中":"更新"}</span>}
          </button>
        </div>
      </div>
      ...
      <div style={{flex:1,overflow:"auto",position:"relative"}}>
        ...
        {page === "main" && (
          <div style={{minHeight:"100vh"}}>
            ...
            {/* Footer */}
            <div style={{position:"fixed",bottom:0,left:48,right:0,padding:"8px 16px",background:"rgba(10,10,15,0.96)",borderTop:"1px solid rgba(255,255,255,0.04)",fontSize:10,color:"#222",display:"flex",justifyContent:"space-between",zIndex:5}}>
              <span>資料來源：撿股讚 Apps Script · 非投資建議</span>
              <span style={{fontFamily:"monospace"}}>v13 · {ALL_STOCKS.length}檔</span>
            </div>
          </div>
        )}
        ...
        {/* ── 新股申購頁 ── */}
        {page === "ipo" && (
          <IPOView ipoData={ipoData} loading={isLoading}/>
        )}

        {/* ── 股東會紀念品頁 ── */}
        {page === "gift" && (
          <GiftView giftData={giftData} loading={isLoading}/>
        )}
